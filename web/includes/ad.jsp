...
<%
    ...
    Criteria ca1 = s.createCriteria(Advert.class).add(Restrictions.eq("adType", ad)).addOrder(Order.desc("id"));
    ...
    if (request.getParameter("cat") != null && !request.getParameter("cat").trim().isEmpty()) {
        String cat = request.getParameter("cat");
        MainCatagory mc = (MainCatagory) s.load(MainCatagory.class, Integer.parseInt(cat));
        ca1.add(Restrictions.eq("mainCatagory", mc));
        
    } else if (request.getParameter("scat") != null && !request.getParameter("scat").trim().isEmpty()) {
        String scat = request.getParameter("scat");
        SubCatagory sc = (SubCatagory) s.load(SubCatagory.class, Integer.parseInt(scat));
        ca1.add(Restrictions.eq("subCatagory", sc));
    }
...
<div class="container-fluid" style="padding: 2px; margin-top: 20px">
    ...
    <%
        ...
        AdType adt = (AdType) s.load(POJOS.AdType.class, 1);
        Criteria ca = s.createCriteria(Advert.class).addOrder(Order.desc("id")).addOrder(Order.desc("id"))
                .add(Restrictions.eq("adType", adt));
        ...
        if (request.getParameter("cat") != null && !request.getParameter("cat").trim().isEmpty()) {
            String cat = request.getParameter("cat");
            MainCatagory mc = (MainCatagory) s.load(MainCatagory.class, Integer.parseInt(cat));
            ca.add(Restrictions.eq("mainCatagory", mc));
        } else if (request.getParameter("scat") != null && !request.getParameter("scat").trim().isEmpty()) {
        ...
        if (ca.list().size() > 0 || ca1.list().size() > 0) {
    %>
